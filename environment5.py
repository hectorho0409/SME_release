...
class Env_toolbox(object):
    def __init__(self, steps, multi_frame, height, width):
        ...
        with self.graph.as_default():
            ...
            self.height = height
            self.width  = width
            self.seq_content = np.empty((self.multi_frame, self.height*self.width*self.steps), dtype=np.float32)
    ...
    def game_over(self):
        self.process_step = 0
        #self.seq_content = np.empty((self.multi_frame, self.height*self.width*self.steps), dtype=np.float64)
    ...
    def do_obmc(self, position_update, motion_update, order_update, current_frame):
        ...
        # get shift map
        shift_map_update = self.shift_toolbox.get_transformed_frame(current_frame, motion_update)
        #shift_map_update = np.reshape(shift_map_update, (self.multi_frame, self.height*self.width))
        
        # update state
        self.critical_point[:,self.process_step,:] = position_update
        self.critical_order[:,self.process_step] = order_update[:,0]
        self.seq_content[:, self.height*self.width*self.process_step: self.height*self.width*(self.process_step+1)] = shift_map_update
        self.process_step += 1
        
        return self.obmc_toolbox.do_obmc(self.critical_point[:,0:self.steps,:], self.critical_order[:,0:self.steps], self.seq_content)
    
    def get_delta_reward(self, mc_frame, current_frame, target_frame):
        # if critical_point queue is not full
        if self.process_step < self.steps:
            input("Error !! You need to complete your game (%d/%d) !!"%(self.process_step, self.steps))
            return
    ...
    def update_test_info(self, predict_frame, current_frame, target_frame, task):
        ...
        # reshape
        predict_frame = np.reshape(predict_frame, (self.height, self.width))*255.
        current_frame = np.reshape(current_frame, (self.height, self.width))*255.
        target_frame = np.reshape(target_frame, (self.height, self.width))*255.
        ...
        # get reward
        total_reward = self.reward_toolbox.get_last_reward(predict_frame/255., current_frame/255., target_frame/255.)
