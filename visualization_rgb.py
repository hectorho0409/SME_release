...
import cv2                                          # save residual image
...
class Visualization(object):
    def __init__(self, height, width, folder_name, task_name, total_frame):
        ...
        if not os.path.isdir("./%s/%s_table"%(folder_name, task_name)):
            ...
            os.mkdir("./%s/%s_table/every_frame"%(folder_name, task_name))
        
        # build text
        file = open('./%s/%s_table/ep_mean_info.txt'%(folder_name, task_name), 'w')
        file.close()
        file = open('./%s/copylast_info.txt'%(folder_name), 'w')
        file.close()
    ...
    def initial(self):
        ...
        self.eva_copylast_mse, self.eva_copylast_psnr, self.eva_copylast_reward, self.eva_copylast_ssim = 0., 0., 0., 0. # copy info
    ...
    def update_mean_info(self, mse, psnr, reward, ssim):
        ...
        file.write('epoch: %i, %s-mse: %f, ' % (self.epoch_num-1, self.task_name, mse))
        file.write('%s-psnr: %f, ' % (self.task_name, psnr))
        file.write('%s-reward: %f, ' % (self.task_name, reward))
        file.write('%s-ssim: %f\n' % (self.task_name, ssim))
        file.close()
    
    def update_copylast_info(self, mse, psnr, reward, ssim):
        
        # save copylast info (table)
        ...
        # save copylast info (total)
        self.eva_copylast_mse += mse
        self.eva_copylast_psnr += psnr
        self.eva_copylast_reward += reward
        self.eva_copylast_ssim += ssim
        
        # update copylast (file)
        if self.epoch_frame_label >= self.total_frame - 1:
            
            self.eva_copylast_mse = self.eva_copylast_mse/self.total_frame
            self.eva_copylast_psnr = self.eva_copylast_psnr/self.total_frame
            self.eva_copylast_reward = self.eva_copylast_reward/self.total_frame
            self.eva_copylast_ssim = self.eva_copylast_ssim/self.total_frame
            
            file = open('./%s/copylast_info.txt'%(self.folder_name), 'a')
            file.write('copy-mse: %f, copy-psnr: %f, copy-reward: %f, copy-ssim: %f\n'%(self.eva_copylast_mse,self.eva_copylast_psnr,self.eva_copylast_reward,self.eva_copylast_ssim))
            file.close()
    ...
    def save_image(self, predict, current, target):
        ...
        # reshape
        current = np.reshape(current, (self.height, self.width-cutwidth, 3))
        predict = np.reshape(predict, (self.height, self.width-cutwidth, 3))
        target = np.reshape(target, (self.height, self.width-cutwidth, 3))
        
        # get residual
        frame1 = cv2.cvtColor(predict.astype("uint8"),cv2.COLOR_BGR2GRAY)*1.
        frame2 = cv2.cvtColor(target.astype("uint8"),cv2.COLOR_BGR2GRAY)*1.
        residual = np.abs(frame1 - frame2)
        ...
        imsave('./%s/%s_table/every_frame/frame_%d_c.png'%(self.folder_name, self.task_name, self.epoch_frame_label), current)
        imsave('./%s/%s_table/every_frame/frame_%d_p.png'%(self.folder_name, self.task_name, self.epoch_frame_label), predict)
        imsave('./%s/%s_table/every_frame/frame_%d_t.png'%(self.folder_name, self.task_name, self.epoch_frame_label), target)
    ...
    def plot_frame_mse_table(self):
        ...
        for i in range(self.total_frame):
            text_pos = 'Frame: %4d, Copylast: %6.4f , Pretrain: %6.4f , Predict: %6.4f\n' % (i, y_copylast[i], y_pretrain[i], y_predict[i])
            file.write(text_pos)
        file.close()
